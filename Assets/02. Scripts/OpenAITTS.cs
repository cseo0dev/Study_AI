...
public class OpenAITTS : MonoBehaviour
{
    ...
    // 이벤트 정의
    public event Action<string> onResponseTTS;  // TTS 재생 시작 이벤트
    public event Action onStopAudio;    // TTS 재생 종료 이벤트
    ...
    private void Start()
    {
        ...
        if (OpenAIManager.Instance != null)
        {
            ...
            OpenAIManager.Instance.onResponseOpenAI += OnResponseOpenAI;
        }
        else
        {
            Debug.LogError("OpenAIManager 인스턴스가 없습니다.");
        }
